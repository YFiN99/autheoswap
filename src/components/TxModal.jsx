...
export default function TxModal({ state, hash, msg, onClose }) {
  // Timer 5 detik otomatis tutup jika sukses
  useEffect(() => {
    if (state === TX.OK) {
  ...
  const body = {
    ...
    [TX.OK]:      () => (
      <>
        <Big>✅</Big>
        <T>Success!</T>
        <D style={{color:'#00ff88'}}>{msg}</D>
        {hash && <Hash h={hash} label="View on Explorer ↗"/>}
...
const Hash = ({ h, label }) => (
  <div style={S.hash} onClick={() => window.open(`${EXPLORER}/tx/${h}`, '_blank')}> {label || h} </div>
);
...
const S = {
  ...
  modal:   { background:'#080f1a',border:'1px solid #1a2d4a',borderRadius:16,width:360,overflow:'hidden',position:'relative' },
  ...
  bar:     { position:'absolute',bottom:0,left:0,height:2,background:'#00d4ff',animation:'shrink 5s linear forwards' }
};